...
(defun pg-test-date (con)
  (cl-flet ((scalar (sql) (car (pg-result (pg-exec con sql) :tuple 0))))
    (with-environment-variables (("TZ" "UTC-01:00"))
      ...
      (let* ((far-future (scalar "SELECT 'infinity'::timestamp"))
             (year (decoded-time-year (decode-time far-future))))
        (should (> year 999999)))
      (let* ((far-past (scalar "SELECT '-infinity'::timestamp"))
             (year (decoded-time-year (decode-time far-past))))
        (should (< year -999999)))
      (let* ((far-future (scalar "SELECT 'infinity'::timestamptz"))
             (year (decoded-time-year (decode-time far-future))))
        (should (> year 999999)))
      (let* ((far-past (scalar "SELECT '-infinity'::timestamptz"))
             (year (decoded-time-year (decode-time far-past))))
        (should (< year -999999)))
...
(defun pg-test-numeric (con)
  (cl-flet ((scalar (sql) (car (pg-result (pg-exec con sql) :tuple 0))))
    ...
    (should (isnan (scalar "SELECT sqrt('NaN'::float4)")))
    (should (isnan (scalar "SELECT sqrt('NaN'::float8)")))
    (should (isnan (scalar "SELECT sin('NaN'::float4)")))
    (should (isnan (scalar "SELECT sin('NaN'::float8)")))
    (should (isnan (scalar "SELECT log('NaN'::float4)")))
    (should (isnan (scalar "SELECT log('NaN'::float8)")))
    (should (eql 4 (scalar "SELECT gcd(12, 8)")))
    ...
    (should (pgtest-approx= (scalar "SELECT asinh(0)") 0.0))
    (should (eql 1.0e+INF (scalar "SELECT cot(0)")))
...
(defun pg-test-bytea (con)
  ...
  (cl-flet ((scalar (sql) (car (pg-result (pg-exec con sql) :tuple 0))))
    ...
    (should (equal (vector (decode-hex-string "DEADBEEF") (decode-hex-string "AAFFEE00FFEE"))
                   (scalar "SELECT ARRAY['\\xDEADBEEF'::bytea,'\\xAAFFEE00FFEE'::bytea]::bytea[]")))
    (should (equal (vector (decode-hex-string "DEADBEEF") pg-null-marker (decode-hex-string "2233445566778899"))
                   (scalar "SELECT ARRAY['\\xDEADBEEF'::bytea,NULL,'\\x2233445566778899'::bytea]::bytea[]")))
    (should (equal (vector (decode-hex-string "0000000FF1CE") (decode-hex-string "ACE0FBA5ED"))
                   (scalar "SELECT '{\"\\\\x0000000FF1CE\",\"\\\\xACE0FBA5ED\"}'::bytea[]")))
...
(defun pg-test-array (con)
  (cl-flet ((scalar (sql) (car (pg-result (pg-exec con sql) :tuple 0))))
    ...
    (should (equal (vector t nil pg-null-marker t)
                   (scalar "SELECT '{true,false,NULL,true}'::bool[]")))
    (should (equal (vector t nil t nil)
                   (scalar "SELECT '{1,0,1,0}'::bit[]")))
    (should (equal (vector t nil pg-null-marker t nil)
                   (scalar "SELECT '{1,0,NULL,1,0}'::bit[]")))
    ...
    (let* ((lowest 50)
           (highest 3000)
           (floats (cl-loop for i from lowest to highest collect (format "%s" (+ i 0.34))))
           (vec (scalar (format "SELECT ARRAY[%s]" (string-join floats ",")))))
      (should (eql (1+ (- highest lowest)) (length vec)))
      (should (pgtest-approx= (+ highest 0.34) (aref vec (- highest lowest)))))
...
(defun pg-test-hstore (con)
  ...
  (when (pg-hstore-setup con)
    ...
    (cl-flet ((scalar (sql) (car (pg-result (pg-exec con sql) :tuple 0))))
      ...
      ;; test parsing of array of hstore (_hstore type)
      (let* ((vec (scalar "SELECT ARRAY[hstore(ARRAY['a','1','b','42']), hstore(ARRAY['fooble','bizzle','42','-55'])]::hstore[]"))
             (hs1 (aref vec 0))
             (hs2 (aref vec 1)))
        (should (eql 2 (length vec)))
        (should (cl-every #'hash-table-p vec))
        (should (eql 2 (hash-table-count hs1)))
        (should (eql 2 (hash-table-count hs2)))
        (should (string= "42" (gethash "b" hs1)))
        (should (string= "bizzle" (gethash "fooble" hs2)))
        (should (string= "-55" (gethash "42" hs2))))
