...
(defun pg-run-tests (conn)
  ...
  (pg-test-numeric conn)
  (pg-test-collation conn)
  (pg-test-xml conn)
...
;; ==============================================
(defun pg-test-result (conn)
  (pg-exec conn "DROP TABLE IF EXISTS resulttest")
  (let ((r1 (pg-exec conn "CREATE TABLE resulttest (a int, b VARCHAR(4))"))
        ...
        (r5 (pg-exec conn "UPDATE resulttest SET b='foob' WHERE a=66"))
        (r6 (pg-exec conn "SELECT b FROM resulttest WHERE a=66"))
        (r7 (pg-exec conn "DROP TABLE resulttest"))
        (r8 (pg-exec conn "SELECT generate_series(1, 10)")))
    (message "==============================================")
   ...
   (pg-exec conn "PREPARE ps42 AS SELECT 42")
   (let ((res (pg-exec conn "EXECUTE ps42")))
     (should (eql 42 (car (pg-result res :tuple 0)))))
   (pg-exec conn "DEALLOCATE ps42")
   ;; Test cursors
   (let ((res (pg-exec conn "BEGIN")))
     (should (string= "BEGIN" (pg-result res :status))))
   (pg-exec conn "CREATE TEMPORARY TABLE cursor_test (a INTEGER, b TEXT)")
   (dotimes (i 10)
     (pg-exec conn (format "INSERT INTO cursor_test VALUES(%d, '%d')" i i)))
   (let ((res (pg-exec conn "DECLARE crsr42 CURSOR FOR SELECT * FROM cursor_test WHERE a=2")))
     (should (string= "DECLARE CURSOR" (pg-result res :status))))
   (let ((res (pg-exec conn "FETCH 1000 FROM crsr42")))
     (should (string= "FETCH 1" (pg-result res :status)))
     (should (eql 1 (length (pg-result res :tuples)))))
   (let ((res (pg-exec conn "CLOSE crsr42")))
     (should (string= "CLOSE CURSOR" (pg-result res :status))))
   (let ((res (pg-exec conn "COMMIT")))
     (should (string= "COMMIT" (pg-result res :status))))
   (let ((res (pg-exec conn "EXPLAIN ANALYZE SELECT 42")))
   ...
   (let ((res (pg-exec conn "SELECT from information_schema.routines")))
...
(defun pg-test-parameter-change-handlers (conn)
