...
(defun pg-test-insert (con)
  (cl-flet ((scalar (sql) (cl-first (pg-result (pg-exec con sql) :tuple 0))))
    (let ((count 100))
      ...
      (cl-loop for i from 1 to count
               for sql = (format "INSERT INTO count_test VALUES(%s, %s)"
                                 i (* i i))
               do (pg-exec con sql))
      ;; CrateDB's "eventually consistent" design means that we need to force a refresh before the
      ;; new inserts become visible to a subsequent query.
      (pcase (pgcon-server-variant con)
        ('cratedb
         (pg-exec con "REFRESH TABLE count_test"))
        ('risingwave
         (pg-exec con "FLUSH")))
    ...
    (pg-exec con "CREATE TABLE w(i BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, v TEXT)")
    ...
    (let ((count 100))
      ...
      (dotimes (i count)
        (pg-exec-prepared con "INSERT INTO count_test VALUES($1, $2)"
                          `((,i . "int4") (,(* i i) . "int4"))))
      ;; This is bizarrely failing in CrateDB.
      (pcase (pgcon-server-variant con)
        ('cratedb
         (pg-exec con "REFRESH TABLE count_test")))
      (should (eql count (scalar "SELECT COUNT(*) FROM count_test")))
...
(defun pg-test-ensure-prepared (con)
  (cl-flet ((scalar (sql) (cl-first (pg-result (pg-exec con sql) :tuple 0)))
    ...
    (pcase (pgcon-server-variant con)
      ('cratedb
       (pg-exec con "REFRESH TABLE prep")))
    ...
    (with-environment-variables (("TZ" "UTC-01:00"))
      ...
      (unwind-protect
          (progn
            ...
            (pcase (pgcon-server-variant con)
              ('cratedb
               (pg-exec con "REFRESH TABLE date_test")))
...
(defun pg-test-hstore (con)
  ...
  (when (pg-hstore-setup con)
    ...
    (cl-flet ((scalar (sql) (car (pg-result (pg-exec con sql) :tuple 0))))
      ...
      (pcase (pgcon-server-variant con)
        ('cratedb
         (pg-exec con "REFRESH TABLE hstored")))
