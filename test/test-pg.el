...
(defun pg-test-insert (con)
  (cl-flet ((scalar (sql) (cl-first (pg-result (pg-exec con sql) :tuple 0))))
    (let ((count 100))
      ...
      (should (string= "pgeltestuser" (pg-table-owner con "count_test")))
      (should (string= "pgeltestuser" (pg-table-owner con (make-pg-qualified-name :name "count_test"))))
...
(defun pg-test-date (con)
  (cl-flet ((scalar (sql) (car (pg-result (pg-exec con sql) :tuple 0))))
    (let (res)
      ...
      (setq res (pg-exec con "SELECT * FROM date_test"))
